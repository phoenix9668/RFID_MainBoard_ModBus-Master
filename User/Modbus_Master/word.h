...
*/
static __INLINE uint16_t lowWord(uint32_t ww)
{
...
*/
static __INLINE uint16_t highWord(uint32_t ww)
{
...
/*模拟ardunio函数*************************************************/
static __INLINE uint8_t lowByte(uint16_t ww)
{
  return (uint8_t) ((ww) & 0x00FF);
}


static __INLINE uint8_t highByte(uint16_t ww)
{
  return (uint8_t) ((ww) >> 8);
}

static __INLINE uint16_t word(uint8_t H_Byte,uint8_t L_Byte)
{
	uint16_t word;
